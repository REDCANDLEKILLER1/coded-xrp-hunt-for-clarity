...
const{CampaignSave}=await load('src/game/definitive/CampaignSave.ts');const{enterWarship,savedChapterScene,chapterLaunchBlock}=await load('src/game/definitive/ChapterTransitions.ts');const{dockFighter}=await load('src/game/definitive/LandingPlan.ts');
...
const oldSave=new CampaignSave(storage,'test:chapter:migration');
oldSave.update(d=>{d.earth.clearedPlanets.push('ledger_prime');d.earth.discoveredPlanets.push('fog_moon');});
const imported=new CampaignSave(storage,'test:chapter:migration');
assert.ok(imported.snapshot.earth.discoveredPlanets.includes('mars'));
assert.ok(imported.snapshot.earth.discoveredPlanets.includes('fog_moon'),'old unlocks survive');
assert.ok(chapterLaunchBlock(imported,'mars'),'discovery cannot launch a missing legacy Mars mission');
assert.equal(chapterLaunchBlock(imported,'ledger_prime'),null,'Earth replay remains available');
imported.update(d=>{d.warshipOwned=true;});
assert.equal(chapterLaunchBlock(imported,'mars'),null,'capture opens the mesh journey');
