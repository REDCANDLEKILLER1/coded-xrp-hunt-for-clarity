...
for(let wave=0;wave<4;wave++){assert.ok(SPACE_WAVES[wave].enemies.length<=3);state.wave=wave;delete state.blockade;if(SPACE_WAVES[wave].capital){assert.equal(clearSpaceWave(save,state).ok,false,'live blockade prevents reward');state.blockade={wave,hp:{shield_port:0,shield_starboard:0,battery_port:0,battery_starboard:0,reactor:0}};}assert.ok(clearSpaceWave(save,state).ok);assert.equal(clearSpaceWave(save,state).ok,false,'reward receipt prevents duplicate salvage');}
