...
const longestDeckSightline=Math.max(...DECK.map(room=>Math.hypot(room.width,room.depth)));
for(const range of [1,longestDeckSightline]){
  const weaponDps=[1,2,3,4].map(level=>boardingWeaponDamagePerSecond(level,range));
  assert.ok(weaponDps.every((damage,index)=>index===0||damage>weaponDps[index-1]),`boarding weapon output must rise at ${range.toFixed(1)}m: ${weaponDps.join(', ')}`);
}
...
const engineeringFixture=new CampaignSave(storage,'test:boarding-engineering');assert.ok(prepareBoardingRoomReview(engineeringFixture,'engineering').ok);assert.equal(engineeringFixture.snapshot.location.checkpoint,'boarding.engineering');assert.ok(engineeringFixture.snapshot.visitedRooms.includes('boarding.rescue'));assert.equal(engineeringFixture.snapshot.clearedRooms.includes('boarding.engineering'),false);
const commandFixture=new CampaignSave(storage,'test:boarding-command');assert.ok(prepareBoardingRoomReview(commandFixture,'command').ok);assert.equal(commandFixture.snapshot.location.checkpoint,'boarding.command');assert.ok(commandFixture.snapshot.quests.includes('boarding.rescue_junction'));assert.ok(commandFixture.snapshot.recruits.includes('mr_zamn'));assert.equal(commandFixture.snapshot.heroUpgrades.boarding_weapon,3);assert.equal(commandFixture.snapshot.clearedRooms.includes('boarding.command'),false);
