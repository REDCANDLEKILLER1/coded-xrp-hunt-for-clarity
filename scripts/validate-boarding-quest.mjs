...
const {selectBoardingTarget,coreExposure,companionPlan,boardingInteraction,canCrossExitField}=await load('src/game/definitive/BoardingCombat.ts');
...
assert.ok(quest.clear('hangar').ok);assert.ok(quest.complete('hangar_safe').ok);
...
assert.ok(quest.enter('rescue').ok);assert.equal(quest.complete('rescue_junction').ok,false,'junction clear required');
assert.ok(quest.clear('rescue').ok);
