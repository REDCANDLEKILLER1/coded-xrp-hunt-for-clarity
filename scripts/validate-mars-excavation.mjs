...
// Interact from the clear south face of the actual rendered pedestal. The old
// gate-center check incorrectly rejected this ordinary approach at z=13.5.
const pedestal=fixture();pedestal.scene.update(.05);pedestal.scene.update(.2);pedestal.scene.comms.dialogue.skip();pedestal.scene.update(.05);for(const guard of pedestal.scene.guards)guard.hp=0;pedestal.scene.hero.position.set(0,0,13.5);pedestal.scene.paint();assert.equal(pedestal.scene.interactButton.textContent,'OPEN GATE');pedestal.scene.interact();assert.ok(pedestal.save.snapshot.quests.includes('mars.route_secured'));assert.equal(pedestal.scene.route,true);pedestal.scene.dispose();
