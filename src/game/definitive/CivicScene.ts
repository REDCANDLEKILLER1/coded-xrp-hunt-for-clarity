...
interface Host{renderer:WebGLRenderer;environment:Texture;root:HTMLElement;quest:BoardingQuest;hero:GLTF;crew:GLTF;deck:GLTF;vendor:GLTF;onBridge:()=>void}
...
export class CivicScene implements ManagedScene{
  ...
  constructor(private readonly host:Host){
    ...
    for(const [color,x,z] of [[0x53776b,0,6],[0x75bcad,-20,5],[0xc28248,-20,-15],[0x8f2424,20,-3]] as const){const l=new PointLight(color,7,10,2);l.position.set(x,2.3,z);this.scene.add(l);}
    ...
    this.input=new SurfaceInput(host.renderer.domElement,this.fire,()=>this.active&&!this.paused&&!this.ui.querySelector('.boarding-shop'),{interact:()=>this.interact(),pause:()=>{this.paused=true;},repair:()=>{},shield:()=>{}});bindFocusPolicy(this.lifetime.signal,()=>this.input.clear(),()=>{if(this.active){this.paused=true;this.input.clear();this.paint();}});host.root.addEventListener('wheel',e=>{if(!this.active||this.ui.querySelector('.boarding-shop'))return;e.preventDefault();this.zoom(Math.sign(e.deltaY));},{signal:this.lifetime.signal,passive:false});this.play('Idle');this.playCrew('Idle');this.updateCamera(true);this.paint();
  }
...
this.mixer.update(dt);this.crewMixer.update(dt);this.updateCamera();this.paint();}
  ...
  dispose():void{this.active=false;this.input.dispose();this.lifetime.abort();this.mixer.stopAllAction();this.crewMixer.stopAllAction();for(const mixer of this.vendorMixers){mixer.stopAllAction();mixer.uncacheRoot(mixer.getRoot());}this.mixer.uncacheRoot(this.host.hero.scene);this.crewMixer.uncacheRoot(this.host.crew.scene);disposeObject(this.scene);this.ui.remove();}
}
