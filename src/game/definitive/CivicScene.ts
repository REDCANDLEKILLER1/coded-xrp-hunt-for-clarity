...
export class CivicScene implements ManagedScene{
  ...
  constructor(private readonly host:Host){
    ...
    const hero=host.hero.scene,crew=host.crew.scene,rested=host.quest.save.snapshot.location.checkpoint==='civic.quarters';hero.position.set(rested?11:0,0,rested?3.2:9.7);crew.position.copy(hero.position).add(new Vector3(-1.3,0,.6));crew.scale.setScalar(.96);this.mixer=new AnimationMixer(hero);this.crewMixer=new AnimationMixer(crew);
    this.scene.background=new Color(0x06111c);this.scene.fog=new Fog(0x06111c,38,78);this.scene.environment=host.environment;this.scene.environmentIntensity=.65;this.scene.add(host.deck.scene,hero,crew,new AmbientLight(0x8fb7ca,.72));
    const sun=new DirectionalLight(0xd9f2ff,2.8);sun.position.set(-12,25,14);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);Object.assign(sun.shadow.camera,{left:-22,right:22,top:22,bottom:-22,near:1,far:70});sun.shadow.camera.updateProjectionMatrix();this.scene.add(sun);
